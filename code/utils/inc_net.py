...
class BaseNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        if not args["init_train"]:
            self.convnet = torch.load(args["convnet_path"])
        else:
            self.convnet = get_convnet(args, False)
        self.fc = None
...
class IncrementalNet(BaseNet):
    ...
    def update_fc(self, nb_classes):
        ...
        elif not self.args["init_train"]:
            fc = torch.load(self.args["fc_path"])
    ...
    def update_fc(self, nb_classes):
        if len(self.convnets) == 0:
            if not self.args["init_train"]:
                self.convnets.append(torch.load(self.args["convnet_path"]))
            else:
...
class FOSTERNet(nn.Module):
    ...
    def forward(self, x):
        out_features = [convnet(x)["features"] for convnet in self.convnets]
        features = torch.cat(out_features, 1)
        # fmaps = [convnet(x)["fmaps"] for convnet in self.convnets]
        out = self.fc(features)
        # fe_logits = self.fe_fc(features[:, -self.out_dim :])["logits"]

        # out.update({"fe_logits": fe_logits, "features": features, "fmaps": fmaps, "out_features": out_features})
    ...
    def update_fc(self, nb_classes, _snet = False):
        if not self.args["init_train"]:
            self.convnets.append(torch.load(self.args["convnet_path"]))
        else:
            self.convnets.append(get_convnet(self.args))
        ...
        if self.fc is not None:
            ...
            fc.bias.data[:nb_output] = bias
            self.convnets[-1].load_state_dict(self.convnets[-2].state_dict())
        elif not self.args["init_train"] and not _snet:
            fc = torch.load(self.args["fc_path"])
...
class TagFexNet(nn.Module):
    ...
    def update_fc(self, nb_classes):
        ...
        if self.fc is not None:
            ...
            fc = torch.load(self.args["fc_path"])
            if isinstance(fc, SimpleLinear):
                new_fc = TagFex_SimpleLinear(fc.in_features, fc.out_features).to(self._device)
                new_fc.weight.data = fc.weight.data
