...
def accuracy(y_pred, y_true, nb_old, init=5, increment=2):
    ...
    # Grouped accuracy
    inc_num = (np.max(y_true) - init + 1) / increment
    class_increment = [increment for i in range(int(inc_num))]
    class_increment.insert(0, init)
    class_id = 0

    for inc in class_increment:
        idxes = np.where(
            np.logical_and(y_true >= class_id, y_true < class_id + inc)
        )[0]
        label = "{}-{}".format(
            str(class_id).rjust(2, "0"), str(class_id + inc - 1).rjust(2, "0")
        )
