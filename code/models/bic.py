...
class BiC(BaseLearner):
    ...
    def _run(self, train_loader, test_loader, optimizer, scheduler, stage, epochs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            average_epoch_time = (average_epoch_time * (epoch - 1) + single_epoch_time) / (epoch)
            if(epoch == 100):
    ...
    def _stage1_training(self, train_loader, test_loader):
        ...
        self._run(train_loader, test_loader, optimizer, scheduler, stage="training", epochs=epochs)
    ...
    def _stage2_bias_correction(self, val_loader, test_loader):
        ...
        optimizer = optim.SGD(
            network_params, lr=self.args["bc_lrate"], momentum=0.9, weight_decay=self.args["weight_decay"]
        )
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer=optimizer, milestones=self.args["milestones"], gamma=self.args["lrate_decay"]
        )
        ...
        self._run(
            val_loader, test_loader, optimizer, scheduler, stage="bias_correction", epochs=self.args["bc_epochs"][self._cur_task]
        )
